 ...
 */
template<typename T = uint64_t>
constexpr T reverse_bytes(T n) noexcept
{
	int l = 0;
	int r = sizeof(T) - 1;
	while(l < r)
 ...
 */
template <typename T = uint64_t>
constexpr T check_bit(const T n, int targ) noexcept
{
 ...
 */
template <typename T = uint64_t>
constexpr T set_bit(const T n, int targ) noexcept
{
  if (targ < 1 || targ > sizeof(n) * BIT_SIZE)
	return n;
 ...
 */
template <typename T = uint64_t>
constexpr T toggle_bit(const T n, int targ) noexcept
{
 ...
 */
template <typename T = uint64_t>
constexpr T clear_bit(const T n, int targ) noexcept
{
  if (targ < 1 || targ > sizeof(n) * BIT_SIZE)
	return n;
 ...
 */
 template<typename T = uint64_t>
 constexpr T flip_bit(const T n, int targ) noexcept
 {
	 if((1 << (targ - 1)) & n)
		 return (~(1 << (targ - 1)) & n);
	 else
...
class Bits
{
	...
	public:
		...
		/*  One param ctor */
		constexpr Bits(const value_type k) noexcept : number(k) {}
		 ...
		 * returns: number of different bits
		 */
		constexpr int hammingDistance(const value_type right) noexcept
		{
		 ...
		 * returns: bool
		 */
		constexpr bool checkBit(value_type n) const noexcept
		{
		 ...
		 * returns: *this
		 */
		constexpr Bits& value(const value_type n) noexcept
		{
		    this->number = n;
		    return *this;
		}
		 ...
		 * returns: *this
		 */
		constexpr Bits& toggleBit(const value_type n) noexcept
		{
		 ...
		 * returns: *this
		 */
		constexpr Bits& setBit(const value_type n) noexcept
		{
		 ...
		 * returns: *this
		 */
		constexpr Bits& clearBit(const value_type n) noexcept
		{
		 ...
		 * returns *this
		 */
		 constexpr Bits& flipBit(const value_type n) noexcept
		 {
		 ...
		 * returns: *this
		 */
		constexpr Bits& add(const value_type num) noexcept
		{
			this->number += num;
			return *this;
		}
		 ...
		 * returns: *this
		 */
		constexpr Bits& subtract(const value_type num) noexcept
		{
			this->number -= num;
			return *this;
		}
		 ...
		 * returns: *this
		 */
		constexpr Bits& multiply(const value_type num) noexcept
		{
			this->number *= num;
			return *this;
		}
		 ...
		 * returns: *this
		 */
		constexpr Bits& divide(const value_type num) noexcept
		{
			this->number /= num;
			return *this;
		}
		 ...
		 * returns: *this
		 */
		constexpr Bits& mod(const value_type num) noexcept
		{
			this->number %= num;
			return *this;
		}
		...
		*/
		...
		constexpr friend Bits<C> operator+ (Bits<G>& left, const Bits<F>& right);
		 ...
		 * 
		template <typename G, typename F, typename C>
		constexpr friend Bits<C> operator+ (Bits<G>& left, const F& right);
		...
		*/
	    ...
	    constexpr friend bool operator==(Bits<G>& left, const Bits<F>& right);
		...
		/* Relational Operators Integral
		 * In progress 
		 *
		template <typename G, typename F>
	    constexpr friend bool operator==(Bits<G>& left, const F& right);
		...
		*/
		...
		Bits& operator*=(const value_type n) noexcept
		{
			this->number *= n;
			return *this;
		}
	...
	private:
		...
		const int size = BIT_SIZE * sizeof(T);	// Number of bits in the object
		int tsize = BIT_SIZE * sizeof(T); // For some special methods
...
constexpr Bits<C> operator+(Bits<G>& left, const Bits<F>& right)
{
...
constexpr bool operator==(Bits<G>& left, const Bits<F>& right)
{
